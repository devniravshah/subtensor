...
impl<T: Config> Pallet<T> {
    ...
    let dividends: Vec<I32F32> = matmul_sparse(&ema_bonds, &incentive, n);
    log::trace!("dividends: {:?}", dividends);

    let mut final_incentives: Vec<I32F32> = incentive.clone();
    inplace_add_vec(&mut final_incentives, &dividends);
    inplace_normalize(&mut final_incentives);
    log::trace!("incentives: {:?}", final_incentives);

    // Incremental record returns.
    let _ = Self::set_ranks(netuid, ranks.clone());
    let _ = Self::set_trust(netuid, trust.clone());
    let _ = Self::set_incentive(netuid, incentive.clone());
    let _ = Self::set_dividends(netuid, dividends.clone());
    let _ = Self::set_bonds_sparse(netuid, ema_bonds.clone());
    let _ = Self::set_weights_sparse(netuid, weights.clone());
    let _ = Self::set_validator_trust(netuid, validator_trust.clone());
    let _ = Self::set_validator_permit(netuid, new_validator_permits.clone());

    log::info!("epoch function executed for netuid: {}", netuid);

    let rao_emission = Self::get_rao_emission();
    let final_incentives_rao: Vec<u64> =
        vec_fixed32_to_u64(vec_scalar_mult(final_incentives, I32F32::from_num(rao_emission)));
    let rewards: Vec<(T::AccountId, u64, u64)> = hotkeys
        .iter()
        .map(|(uid_i, hotkey)| {
            (
                hotkey.clone(),
                final_incentives_rao[*uid_i as usize],
                stake[*uid_i as usize].to_num::<u64>(),
            )
        })
        .collect();
